...
supabase: Client = create_sync_client(
    supabase_url, supabase_key
)  # Sync client for DB writes
...
def draw_boxes(frame: np.ndarray, results) -> np.ndarray:
    ...
    person_boxes = [
        box for box in results[0].boxes if int(box.cls) == 0
...
def store_person_count_data(
    stream_id: str, user_id: Optional[str] = None, name: Optional[str] = None
):
    ...
    if stream_id in person_count_data and person_count_data[stream_id]:
        ...
        try:
            ...
            logging.info(
                f"Data successfully stored in Supabase for stream ID: {stream_id}. Response: {response}"
            )
        ...
        logging.warning(
            f"No person count data found in memory for stream ID: {stream_id} to store."
        )
...
def get_current_person_count(stream_id: str) -> int:
    ...
    if stream_id in person_count_data and person_count_data[stream_id]:
        return person_count_data[stream_id][-1][
            1
        ]  # Get count from the last recorded tuple
    return 0
...
async def shutdown_event():
    ...
    for device, (cap, stream_id, _, _) in list(
        connected_cameras.items()
    ):  # Iterate over a copy
        logging.info(
            f"Releasing camera capture for device {device} (Stream ID: {stream_id}) on shutdown."
        )
        cap.release()
    ...
    try:
        while True:
            ...
            try:
                ...
                logging.debug(
                    f"Received message from WebSocket {stream_id}: {message[:50]}..."
                )  # Log snippet
        ...
        logging.error(
            f"WebSocket connection closed for stream ID: {stream_id}. Error: {e}"
        )
    finally:
...
async def ipcam_endpoint(
    device: str = Query(
        ..., description="IP address and port of the camera (e.g., 192.168.1.10:4747)"
    ),
    ...
    logging.info(
        f"Attempting to connect to video stream: {video_url} for Stream ID: {id}"
    )
    ...
    if not cap.isOpened():
        ...
        raise HTTPException(
            status_code=503, detail=f"Unable to open video stream at {video_url}"
        )

    if device in connected_cameras:
        logging.warning(
            f"Device {device} already has an active stream. Closing existing one."
        )
    ...
    logging.info(
        f"Stream started successfully for Device: {device}, Stream ID: {id}, Camera Name: {name}"
    )
    ...
    async def generate():
        ...
        try:
            while cap.isOpened():
                ...
                if not ret:
                    logging.warning(
                        f"Failed to retrieve frame from {video_url}. Stream ending."
                    )
                    break
                ...
                person_count = sum(
                    1 for r in results[0].boxes if int(r.cls) == 0
                )  # Class 0 is 'person'
                ...
                await asyncio.sleep(
                    0.05
                )  # ~20 FPS target, adjust based on processing time

        except Exception as e:
            logging.error(
                f"Error during video generation for stream {stream_id} ({video_url}): {e}"
            )
        finally:
            logging.info(
                f"Closing video stream and cleaning up for device {device} (Stream ID: {stream_id})."
            )
            cap.release()
            if device in connected_cameras:
                ...
                if stored_id == stream_id:
                    ...
                    logging.warning(
                        f"Device {device} mapping changed unexpectedly during stream."
                    )
...
async def stop_stream(data: Dict[str, str]):
    ...
    if not device:
        raise HTTPException(
            status_code=400, detail="Missing 'device' (IP:Port) in request body"
        )

    if device in connected_cameras:
        cap, stream_id, user_id, name = connected_cameras[device]
        logging.info(
            f"Stopping stream requested for device {device} (Stream ID: {stream_id})."
        )
        cap.release()
        del connected_cameras[device]
        ...
        logging.info(
            f"Storing final data for stream ID: {stream_id} due to stop request."
        )
        store_person_count_data(stream_id, user_id, name)
        # Note: person_count_data[stream_id] is deleted inside store_person_count_data on success

        return {
            "message": f"Stream for device {device} (ID: {stream_id}) stopped successfully and data stored."
        }
    else:
        logging.warning(
            f"Stop stream request: No active stream found for device {device}."
        )
        raise HTTPException(
            status_code=404,
            detail=f"No active stream found for the specified device IP: {device}",
        )
